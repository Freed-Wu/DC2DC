...
int dot = 0;
int num=0;
volatile unsigned int samplevalue;
...
void main( void )
{
	...
	AdcRegs.ADCMAXCONV.bit.MAX_CONV1 = 0x0;// convert and store in 1 results registersDELAY_US(100);
	...
	while ( 1 )
	{
		...
		********/
	    if (lcdflag==1)
	    {
	        ...
	        char show3[12] = "";
	        num2char(show3, (adc0-2.1)/0.185, 1, 3);
	        char show31[12]  = "电流：";
	        strcat(show31, show3);
	        strcat(show31, "A");
...
interrupt void cpu_timer0_isr(void)
{
    ...
    samplevalue=Ad_Get(0);
    samplesave[adcindex]=samplevalue;
    adcindex++;
    if(adcindex==10)
    {
        adcindex=0;
        adc0=(float)(samplesave[0]+samplesave[1]+samplesave[2]+samplesave[3]+samplesave[4]+samplesave[5]+samplesave[6]+samplesave[7]+samplesave[8]+samplesave[9])* 3.0 /4096.0/10.0;
    }
    /*f (adcflag==1)
    {
        adc0=adc1;
